...
На 22:30 по Москве все баги, кроме шестого, ловятся. Номер 7 и номер 9 ловятся двумя тестами. Один из тестов ловит аж три бага ('после отправки инфо оформляется заказ'), не знаю, что с ним делать, ключевой момент все-таки. Баги номер 1, 2, 3 и 9 ловятся Гермионой, остальные юнитами. А! Шестой вообще не ловится, попробую до дедлайна что-то сделать, но не уверена.

Для удобства я поудаляла все тесты, которые не проверяли баги (если интересно посмотреть, я их сохранила, но там помойка).

На притиер и вылизать тайпскрипт меня не хватило, извините.

Запускать предполагается сначала без багов (чтоб у гермионы скрины настроить).

Я проверяла, запуская по очереди юнит тесты и затем е2е, потому что Гермиона все время что-то пишет, я замучалась пролистывать до юнитов.

Если вдруг что не пойдет (а оно может), свяжись со мной, я видео запишу с проверкой.

tg @verozzy

Хром Version 114.0.5735.133 (Official Build) (64-bit)
Система Ubuntu 20.04.6 LTS (Focal Fossa)
